...
void main() {
  group('PdfTextExtractor', () {
    test('isPdfContent detects PDF magic bytes', () {
      final pdfBytes = Uint8List.fromList('%PDF-1.4 fake content'.codeUnits);
      expect(PdfTextExtractor.isPdfContent(pdfBytes), isTrue);
    });

    test('isPdfContent rejects non-PDF bytes', () {
      final htmlBytes = Uint8List.fromList('<html>not a pdf</html>'.codeUnits);
    ...
    test('isPdfContentType detects with charset', () {
      expect(
        PdfTextExtractor.isPdfContentType('application/pdf; charset=utf-8'),
        isTrue,
      );
    });
    ...
    test(
      'extract handles process with large stderr without deadlock',
      () async {
