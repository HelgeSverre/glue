...
void main() {
  group('ServiceLocator.create', () {
    test('BUG-002: does not create an empty session directory on startup '
        '(so --resume does not leave a stale empty session behind)', () async {
      final tempDir = Directory.systemTemp.createTempSync(
        'service_locator_bug002_',
      );
      ...
      expect(SessionStore.listSessions(env.sessionsDir).map((s) => s.id), [
      ...
      expect(
        ...
        ['pre-existing-session'],
        reason:
            'ServiceLocator must not eagerly persist an empty session; '
    ...
    test(
      'installs OTEL exporter and enables auto-flush when configured',
      () async {
        final tempDir = Directory.systemTemp.createTempSync(
          'service_locator_otel_',
        );
        addTearDown(() => tempDir.deleteSync(recursive: true));
        ...
        final env = Environment.test(
          ...
          vars: {
            ...
            'OTEL_EXPORTER_OTLP_ENDPOINT':
                'https://collector.example.test/root',
