...
void main() {
  group('openInFileManager', () {
    test(
      'invokes the platform-appropriate launcher for a valid directory',
      () async {
        String? capturedExe;
    ...
    test(
      'returns false and skips launch when the directory is missing',
      () async {
        var launched = false;
        final ok = await openInFileManager(
          '/tmp/does-not-exist',
    ...
    test('returns false when the launcher exits non-zero', () async {
      final ok = await openInFileManager(
        '/tmp/glue-home',
        directoryExists: (_) => true,
        runner: (_, _) async => ProcessResult(0, 1, '', 'launch failed'),
      );
      expect(ok, isFalse);
    });
