...
class _CapturingClient extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    return http.StreamedResponse(Stream.value(utf8.encode(ndjson)), 200);
  }
}
...
void main() {
  group('OllamaClient message mapping', () {
    ...
    test(
      'tool_result with contentParts but no images uses text only',
      () async {
        final messages = [
          Message.user('hi'),
          Message.assistant(
          ...
          Message.toolResult(
            ...
            content: 'fallback',
            toolName: 'read_file',
            contentParts: [const TextPart('file contents here')],
          ),
        ];
