...
void main() {
  ...
  group('WorkspaceBootstrap via BootstrapBundleTransport', () {
    test(
      'takes the bundle path on a non-git host (no remote needed)',
      () async {
        ...
        expect(
          File('${sandboxDir.path}/workspace/main.dart').existsSync(),
          isTrue,
        );
      },
    );
    ...
    test(
      'captures uncommitted host changes (user\'s broken scenario)',
      () async {
        ...
        final inSandbox = File(
          '${sandboxDir.path}/workspace/feature.dart',
        ).readAsStringSync();
        expect(
          inSandbox,
          '// v2 wip\n',
          reason: 'sandbox must see the uncommitted edit, not the committed v1',
        );
      },
    );

    test(
      'raises BootstrapException(clone-bundle) when sandbox clone fails',
      () async {
    ...
    test(
      'deletes the host-side bundle file when size cap forces fallback',
      () async {
        ...
        final transport = _FakeBundleTransport(
          sandboxDir,
        ...
        expect(
          ...
          isFalse,
          reason:
              'bundle file should be deleted when size cap forces '
              'fallback',
        );
      },
    );
...
class _FakeBundleTransport implements BootstrapBundleTransport {
  ...
  Future<BootstrapExecResult> run(String shellCommand) async {
    ...
    final rewritten = shellCommand.replaceAll(
      '/tmp/glue-bootstrap.bundle',
