...
void main() {
  group('WebSocketTransport', () {
    test(
      'round-trips JSON-RPC messages through a real WS connection',
      () async {
        ...
        // Wire server: echo
        unawaited(
          acceptedTransport.then((t) {
            t.incoming.listen((msg) {
              if (msg is JsonRpcRequest) {
                t.send(
                  JsonRpcResponse(id: msg.id, result: 'echo:${msg.method}'),
                );
              }
            });
          }),
        ...
        final reply = await clientTransport.incoming.firstWhere(
    ...
    test('decodes inbound binary frames as UTF-8 JSON', () async {
      ...
      unawaited(
        _serverTransport(server).then((t) {
      ...
      client.add(utf8.encode('{"jsonrpc":"2.0","id":7,"method":"binary"}'));
