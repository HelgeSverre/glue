...
void main() {
  group('McpAuthFlowRunner', () {
    test(
      'emits Discovering → Registering → AwaitingCallback → Success when DCR needed',
      () async {
        ...
        final runner = McpAuthFlowRunner(
          ...
          codeFlow:
              ({
                ...
                return const OAuthTokens(accessToken: 'AT', refreshToken: 'RT');
              },
        );
    ...
    test(
      'errors when no registration_endpoint and no stored client_id',
      () async {
        final fakeHttp = _FakeHttpClient({
          Uri.parse('https://meta.example/protected'): const _Response(
            200,
