...
> **2026-04-29 addendum.** The original plan (below) predates the
> harness-layers refactor. The ACP server now lives in
> `packages/glue_server/lib/src/acp/`, not `cli/lib/src/acp/`. The ACP
> session is a *new surface* over the same harness API the CLI consumes
> — it imports `package:glue_harness/glue_harness.dart` for the agent
> loop, `package:glue_core/glue_core.dart` for the typed event/command
> vocabulary, and never reaches into either's `src/`. The CLI grows a
> `glue serve --stdio` subcommand that constructs and runs the server.
> The web-UI half of the original plan (Approach A vanilla JS,
> stdio-to-ws bridge) is unchanged. See
> `docs/plans/2026-04-29-harness-layers.md` for the package layout and
> `docs/plans/2026-04-29-mcp-client.md` for the sibling MCP work.
