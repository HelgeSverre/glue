...
- [ ] **C1 — Cancelling/denying a tool mid-flight corrupts the conversation → next API call 400s.**
...
- [ ] **C2 — `glue mcp add` silently deletes `mcp.tool_policy` (deny-list) and sibling `mcp.*` keys/comments.**
...
- [ ] **C3 — Model/tool/thinking output rendered to the terminal with no ANSI sanitization.**
...
- [ ] **C4 — SSRF via auto-allowed `web_fetch` → cloud-metadata credential theft.**
...
- [ ] **H5 — Corrupt catalog cache bricks every `glue` invocation.** `loadYaml` throws
...
- [ ] **H6 — Terminal not restored on panic / SIGTERM / SIGHUP.** `cli/bin/glue.dart:229`
  ...
  SIGTERM/SIGHUP.*
- [ ] **H7 — ACP WebSocket: no `Origin` check + loopback needs no token → drive-by localhost RCE.**
  ...
  *Fix: reject requests carrying an `Origin` header (real editor clients don't send one);
  require the token even on loopback.*
- [ ] **H8 — ACP client error-reply to a permission request hangs the prompt forever.**
...
- [ ] **H9 — ACP pending permissions never failed on disconnect.** `server.dart:112-120`
...
- [ ] **H10 — ACP host: un-awaited, un-caught `_runConnection`/accept futures can kill the isolate / stop accepting.**
  `http_host.dart:76-102`. *Fix: catch in both `_runConnection` and the accept loop.*
- [ ] **H11 — MCP pool leaks child processes on connect failure.** `pool.dart:357-367`
...
- [ ] **H12 — MCP double-connect race + can't disable a failing server.** `pool.dart:304`
...
- [ ] **H13 — Daytona: killing one background command kills them all + breaks future ones.**
...
- [ ] **H14 — Daytona: `handle.exitCode.then(...)` with no `onError` → isolate-fatal unhandled error.**
  `glue_runtimes/.../common/transport_executor.dart:112`. *Fix: add `onError`.*
- [ ] **H15 — Sprites bundle upload E2BIG far below the 3 MB cap, hard-fails bootstrap.**
...
- [ ] **M1 — MCP `call_timeout_seconds` parsed everywhere but never applied** (30s hard cap). `pool.dart:118-147` / `client.dart:74`.
...
- [ ] **M18 — `--json` emits no output at all on error.** `app.dart:2334-2345`. *Fix: emit a JSON envelope with an `error` field.*
- [ ] **M19 — Blocking `! cmd` buffers unbounded output → OOM/hang.** `app.dart:2643-2664` `.join()` with no cap. *Fix: cap bytes, truncate with notice.*
- [ ] **M20 — Ollama pull-confirm modal orphaned if user exits while open.** `app.dart:745-819`. *Fix: guard post-await UI mutations with `_exitCompleter.isCompleted`.*
- [ ] **M21 — `_render()` throttle can drop the final frame → stale UI.** `app.dart:824-839`. *Fix: always `_doRender()` in the trailing callback, or a `_dirty` flag.*
- [ ] **M22 — `_mode`/`_activeModal` pairing hand-maintained in ~8 places, no invariant.** `app.dart`. *Fix: derive `confirming` from `_activeModal != null`.*
- [ ] **M23 — Daytona client-side timeouts don't cancel remote work and escape the typed error contract.** `daytona/client.dart:393-395`, `sprites/cli.dart:150-153,200-202` (leaked subprocess).
- [ ] **M24 — Failed checkout leaves a poisoned workspace that later reports `resumed: true`.** `common/bootstrap.dart:277-279`. *Fix: `rm -rf` on checkout failure or verify HEAD on resume.*
- [ ] **M25 — `classifyCloneFailure` misclassifies "Repository not found" (private-repo auth) as missing-git-binary.** `common/bootstrap.dart:211-219`.
...
- [ ] **L11 — MCP request timeout doesn't send `notifications/cancelled`; stdio SIGTERMs without closing stdin first.** `client.dart:196-205`, `stdio.dart:116-134`.
- [ ] **L12 — Tool-schema conversion crashes the whole server on list-typed `type` / missing name.** `tool_factory.dart:125`, `protocol.dart:141`. *Fix: accept list types; skip malformed descriptors.*
