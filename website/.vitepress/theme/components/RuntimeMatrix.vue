...
<template>
  <div class="rm-wrap">
    <table class="rm-table">
      <caption v-if="caption" class="rm-caption">
        {{
          caption
        }}
      </caption>
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.runtime">
          <th scope="row" class="rm-row-head">
            ...
            <div v-if="row.status" class="rm-status" :data-status="row.status">
              {{ row.status }}
            </div>
          ...
          <td
            v-for="cap in capabilities"
            :key="cap"
            :data-cell="row.capabilities[cap] ?? 'no'"
            :title="`${row.runtime} · ${cap}: ${row.capabilities[cap] ?? 'no'}`"
          >
...
.rm-status[data-status="shipping"] {
  color: var(--glue-success);
}
.rm-status[data-status="experimental"] {
...
.rm-table td[data-cell="yes"] {
  color: var(--glue-success);
  font-weight: 600;
}
.rm-table td[data-cell="partial"] {
  color: var(--glue-accent);
}
.rm-table td[data-cell="planned"] {
  color: var(--vp-c-text-3);
}
.rm-table td[data-cell="no"] {
  color: var(--vp-c-text-3);
}
