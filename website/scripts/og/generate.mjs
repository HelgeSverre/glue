...
function fitHeadline(
  text,
  { maxWidth, minSize, maxSize, step = 4, maxLines = 2 },
) {
...
function tspansFromLines(lines, firstY, lineHeight) {
  return lines
    .map(
      (line, i) =>
        `<tspan x="80" y="${firstY + i * lineHeight}">${esc(line)}</tspan>`,
    )
    .join("");
}
...
const DEFAULTS = {
  ...
  subtitle:
    "Edits files, runs shell, drives a browser when the task calls for it.",
};
...
const ROUTE_OVERRIDES = {
  "/": {
    ...
    subtitle:
      "Edits files, runs shell, drives a browser. Runs on your host or in a Docker sandbox.",
  },
};
...
async function main() {
  ...
  for (const abs of mdFiles) {
    ...
    const title =
      override.title ?? data.title ?? extractH1(content) ?? DEFAULTS.title;
  ...
  console.log(
    `✓ generated ${results.length} OG images in ${path.relative(WEBSITE_ROOT, OUT_DIR)}/`,
  );
}
